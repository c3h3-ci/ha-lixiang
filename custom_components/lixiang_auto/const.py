...
# 手动凭据流程可覆盖) ----------
# ============================================================================
# ★★★ 签名凭据（必须由使用者自行提取，不能内置）★★★
# ============================================================================
#
# 这三个凭据【不是 App 常量】，而是【每台设备独有的】：
#
#   · hac_key  —— x-chj-sign 的 HMAC 密钥
#                 运行期由理想服务端下发后本地解密存储
#                 Android: SharedPreferences k_c_1_4_1_2 的 k11
#                 iOS:     Keychain（受访问组密钥加密）
#                 提取方式：越狱设备 Hook CCHmac，或 root 设备读 MMKV
#
#   · key_id   —— x-chj-key（签名身份）
#                 App 安装时客户端生成一次并绑定
#                 同一账号在不同设备上 keyId 不同
#
#   · xdev     —— X-CHJ-Deviceid（设备指纹）
#                 同样是设备独有，服务端按它校验签名
#
#   · app_token —— X-CHJ-TOKEN（短效，会过期）
#
# 为什么不能内置？
#   服务端按 (keyId + deviceId) 校验签名。
#   如果内置了某台设备的凭据，所有用户都会被识别为【同一台设备】，
#   既无法工作（签名校验失败），也有隐私和风控风险。
#
# 如何配置？
#   在 config_flow 的「手动凭据」入口填入你自己设备的这四个值。
#   提取方法见 README 的「提取签名凭据」章节。
# ============================================================================

DEFAULT_HAC_KEY = ""     # 使用者自行填写
DEFAULT_KEY_ID = ""      # 使用者自行填写
DEFAULT_XDEV = ""        # 使用者自行填写
DEFAULT_APP_TOKEN = ""   # 使用者自行填写（可留空）
