...
> | 单元测试 | ✅ **118 个**（signals / rendering / policy / coordinator / features）<br>❓ 实车端到端仍需手工验证 |
>
> ### 已知问题
>
> - 部分信号语义仍在核实（`charge_gun_ac` / `ChargeStatus` 等）
> - 尾门状态未实现 App 的「锁优先」聚合规则
> - 车控命令超时后状态可能延迟一轮轮询才更新
...
- **实时状态**（144 个信号）：电量、续航、门锁、车窗、胎压、温度、
...
**135 个实体**（L6 实测；其他车型按功能探测自动增减），按平台分类：

| 平台 | 数量 | 说明 |
|---|---|---|
| sensor | 68 | 电量、续航、温度、胎压、充电… |
| binary_sensor | 35 | 门锁、车窗、充电枪、告警… |
| button | 11 | 寻车、闪灯、鸣笛、哨兵开关、远程拍照… |
| **fan** | **8** | **座椅加热/通风（关闭·低·中·高，含二排）** |
| switch | 5 | 方向盘加热、快热、快冷、除霜、充电启停 |
| **cover** | **2** | **尾门 / 全车窗** |
| climate | 1 | 空调（温度/模式）|
| lock | 1 | 车锁 |
| number | 1 | 空调设定温度 |
...
> 💡 座椅用 `fan` 域（而非 switch+number），因为 HA 的 fan 原生支持
> 「关闭/低/中/高」档位 UI；尾门/车窗用 `cover` 域，状态与操作合一。
